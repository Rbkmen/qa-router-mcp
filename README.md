...
Локальный STDIO MCP-сервер для Codex Desktop. Gemma создаёт черновики QA-артефактов и ограниченных текстовых задач, а Hermes получает только явно подтверждённые обезличенные предпочтения.
...
- `translate_text` — перевод обезличенного текста с сохранением указанных терминов.
- `rewrite_text` — сокращение, исправление или изменение стиля без добавления фактов.
- `explain_short` — краткое объяснение стабильной темы без внешнего исследования.
- `summarize_text` — выжимка только из переданного текста с опциональным фокусом.
