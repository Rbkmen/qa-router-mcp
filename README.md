...
- `record_canary_feedback` — обезличенная оценка проверенного локального черновика;
  принимает только тип маршрута, результат проверки и категорию основной правки.
...
Router v6 выбирает фактический выходной бюджет ниже этого потолка:
...
События Router v6 помечаются моделью, версией профиля и источником
...
Для первых 50 успешно проверенных interactive-черновиков Codex записывает ровно
одну оценку: `accepted`, `edited` или `rejected` и, при правке/отклонении, одну
категорию причины. Текст задачи и черновика, Jira-ключи и другие идентификаторы в
feedback не попадают. Прогресс canary считается за всё время отдельно от
семидневных generation-метрик; после 50 оценок router перестаёт их запрашивать.
